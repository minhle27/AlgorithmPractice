...
bool isOkay(vi cow_order, vi pos, vi check) {
    int x = 1;
    for (int kid = 0; kid < m; kid++) {
        if (pos[cow_order[kid]]) {
            if (pos[cow_order[kid]] < x) {
                return false;
            }
            x = pos[cow_order[kid]];
        } else {
