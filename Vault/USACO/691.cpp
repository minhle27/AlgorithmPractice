...
int main()
{
    ...
    vi P(n), H(n), S(n);
    int total_P = 0, total_H = 0, total_S = 0;
    for (int i = 0; i < n; i++) {
        char move;
        cin >> move;
        if (move == 'P') {
    ...
    int res = 0;
    int cnt_P = 0, cnt_H = 0, cnt_S = 0;
    for (int i = 0; i < n; i++) {
        int half1_win = i == 0 ? 0 : max({cnt_P, cnt_H, cnt_S});
        int half2_win = max({total_P - cnt_P, total_H - cnt_H, total_S - cnt_S});
        ckmax(res, half1_win + half2_win);
        cnt_P += P[i]; cnt_H += H[i]; cnt_S += S[i];
    }
    cout << res << endl;
