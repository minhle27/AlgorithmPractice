...
vi cows(maxn + 7), bread1(maxn + 7, 0), bread2(maxn + 7, 0), bread3(maxn + 7, 0);

void build_pref() {
    for (int i = 1; i <= n; i++) {
        bread1[i] = bread1[i - 1] + (cows[i] == 1);
        bread2[i] = bread2[i - 1] + (cows[i] == 2);
        bread3[i] = bread3[i - 1] + (cows[i] == 3);
    }
}
...
int main()
{
    ...
    for (int i = 1; i <= q; i++) {
        int a, b;
        cin >> a >> b;
        int type1 = bread1[b] - bread1[a - 1];
        int type2 = bread2[b] - bread2[a - 1];
        int type3 = bread3[b] - bread3[a - 1];
