...
int area(vi& rec) {
    int l1 = max(rec[0], rec[2]) - min(rec[0], rec[2]);
    int l2 = max(rec[1], rec[3]) - min(rec[1], rec[3]);
    return l1 * l2;
}

int overlap(vi& rec1, vi& rec2) {
    int l1 = max(min(rec1[2], rec2[2]) - max(rec1[0], rec2[0]) , 0);
    int l2 = max(-max(rec1[1], rec2[1]) + min(rec1[3], rec2[3]) , 0);
    return l1 * l2;
}
...
int main()
{
    ...
    cout << area(recs[1]) + area(recs[2]) - overlap(recs[3], recs[1]) - overlap(recs[3], recs[2]) << endl;
