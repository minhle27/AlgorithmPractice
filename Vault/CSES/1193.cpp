...
vector<vi> d(maxn, vi(maxn, 0));
vector<vi> path(maxn, vi(maxn, -1));
...
void bfs(pi s) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 0; i < 4; i++) {
            int u = x + dx[i];
            int v = y + dy[i];
            if (isValid(u, v) && maze[u][v] != '#' && !visited[u][v]) {
                q.push({u, v});
                visited[u][v] = true;
                d[u][v] = d[x][y] + 1;
                path[u][v] = i;
...
int main()
{
    ...
    for (int i = 0; i < n; i++) {
        for (int j = 0; j < m; j++) {
            cin >> maze[i][j];
            if (maze[i][j] == 'A') start = pi {i, j};
            else if (maze[i][j] == 'B') end = pi {i, j};
        }
    }
    ...
    if (visited[end.first][end.second]) {
        ...
        vi steps;
        while (end != start) {
            int i = path[end.first][end.second];
