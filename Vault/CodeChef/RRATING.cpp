...
int main()
{
    ...
    priority_queue<int, vi, greater<int>> onethird;
    int listSize = 0;
    cin >> n;
    for (int i = 0; i < n; i++) {
        int type;
        ...
        if (type == 1) {
            ...
            cin >> rating;
            listSize++;
            if (sz(onethird) && rating > onethird.top()) {
                list.push(onethird.top());
                onethird.pop();
                onethird.push(rating);
            } else {
                list.push(rating);
            }
            if (listSize > 0 && listSize % 3  == 0) {
                onethird.push(list.top());
                list.pop();
            }
        } else {
            if (sz(onethird) == 0) {
