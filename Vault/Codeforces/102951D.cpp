...
vector<vi> updates;
vector<pi> queries;
vector<ll> diff(400005);
vi widths(400005);
vector<ll> interval_value(400005);
...
int main()
{
    ...
    for (int i = 0; i < N; i++) {
        vi update = updates[i];
        diff[getCompressedIndex(update[0]) + 1] += update[2];
        diff[getCompressedIndex(update[1]) + 1] -= update[2];
    }
	...
	for (int i = 1; i < sz(indices); i++) {
		interval_value[i] = interval_value[i - 1] + diff[i];
	}

	vector<ll> prefix_sums(400005);
	for (int i = 1; i < sz(indices); i++) {
