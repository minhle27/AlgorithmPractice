...
int main()
{
    ...
    for (int i = 1; i <= sz(points); i++) {
        compress[points[i - 1]] = i;
        actual[i] = points[i - 1];
    }

    vector<ll> dif(sz(points) + 1);
    for (int i = 0; i < sz(updates); i++) {
        dif[compress[updates[i].first]]++;
        dif[compress[updates[i].second]]--;
    }

    vector<ll> res(n + 1);
    for (int i = 1; i < sz(points); i++) {
        dif[i] += dif[i - 1];
        res[dif[i]] += actual[i + 1] - actual[i];
    }
