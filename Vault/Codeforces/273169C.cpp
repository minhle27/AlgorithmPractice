...
Node operator + (const Node& left, const Node& right) {
    int tmp_min = min(left.min, right.min);
    int tmp_cnt = 0;

    if (tmp_min == left.min && tmp_min == right.min) {
        tmp_cnt = left.cnt + right.cnt;
    }
    else if (tmp_min == left.min) {
        tmp_cnt = left.cnt;
    }
    else {
        tmp_cnt = right.cnt;
...
int main()
{
    ...
    for (int i = 0; i < m; i++) {
        ...
        if (t == 1) {
            ...
            Node res = get(1, 1, n, a + 1, b);
            cout << res.min << " " << res.cnt << endl;
        }
    }
