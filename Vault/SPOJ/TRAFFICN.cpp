...
vector<vector<pi>> g;
vector<vector<pi>> rev_g;
vi distS;
vi distT;
const int INF = 1e9 + 7;

void dijkstra(int s, vi &d, vector<vector<pi>> &g) {
    d[s] = 0;
...
int main()
{
    ...
    while (test--) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            int opt1 = distS[u] + q + distT[v];
            int opt2 = distS[v] + q + distT[u];

            if (distS[u] == INF || distT[v] == INF) opt1 = INF;
            if (distS[v] == INF || distT[u] == INF) opt2 = INF;

            ckmin(res, min(opt1, opt2));
        }
