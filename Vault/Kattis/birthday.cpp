...
void dfs(int s, pi edge) {
    visited[s] = true;
    for (auto u : g[s]) {
        if (pi {s, u} == edge || pi {u, s} == edge) continue;
        if (!visited[u]) dfs(u, edge);
    }
}

int find_comps(pi edge) {
    int res = 0;
    for (int i = 0; i < p; i++) {
        if (!visited[i]) {
            dfs(i, edge);
    ...
    visited = vector<bool> (maxp, false);
    return res;
}
...
int main()
{
    ...
    while (true) {
        ...
        int comps = find_comps({-1, -1});

        bool flag = false;
        for (auto edge: edges) {
            int cur = find_comps(edge);
            if (cur != comps) flag = true;
        }
