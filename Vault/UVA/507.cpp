...
pi maxRSQ(vi &n, vi &trace) {
    int sum = 0, cur = 0, end = 0, mxl = 0;
    bool newStart = true;
    for (int i = 1; i <= s - 1; i++) {
        sum += n[i];
        if (newStart) trace[i] = i;
        else trace[i] = trace[i - 1];
...
int main()
{
    ...
    for (int r = 1; r <= b; r++) {
        cin >> s;
        vi n(s + 1), trace(s + 1, -1);
        for (int i = 1; i <= s - 1; i++) {
            cin >> n[i];
        }
        pi res = maxRSQ(n, trace);
        int end = res.first;
        int start = trace[end];
        int sum = res.second;
        if (sum <= 0) cout << "Route " << r << " has no nice parts" << endl;
        else cout << "The nicest part of route " << r << " is between stops " << start << " and " << end + 1 << endl;
    }
